...
if (!isset($msg_nom) || empty($msg_nom)) {
    $ok = false;
    array_push($msg, htmlspecialchars('Le champ nom doit etre renseigner !'));
    // $msg[] = ;
}

if (!isset($msg_prenom) || empty($msg_prenom)) {
    $ok = false;
    $msg[] = htmlspecialchars('Le champ prenom doit etre renseigner !');
}

if (!isset($msg_email) || empty($msg_email)) {
    $ok = false;
    $msg[] = htmlspecialchars('L\'email doit etre renseigner !');
}

if (!isset($msg_password) || empty($msg_password)) {
    $ok = false;
    $msg[] = htmlspecialchars('Le mot de passe doit etre renseigner !');
}
...
if ($log_user) {
    $ok = true;
    $msg[] = htmlspecialchars('Connexion réussie');
} else {
    $ok = false;
    $msg[] = htmlspecialchars('Connexion échoué');
}
